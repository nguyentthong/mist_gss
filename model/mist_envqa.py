...
from transformers import DistilBertConfig, BertConfig, BertTokenizer, BertModel
import clip
...
class ISTA(nn.Module):
    def __init__(self, feature_dim, word_dim, Q, N, d_model, dropout, d_ff, h, topk=6, topj=12, use_gss=0):
...
class MIST_VideoQA(nn.Module):
    ...
    def get_answer_embedding(self, answer):
        answer, answer_g = self.amodel(answer)
        return answer, answer_g
    ...
    def forward(
        ...
        video_o = self.encode_vid(video_o)
        if self.use_gss:
            video_o = self.gss(video_o.view(bsize, numc * numf * numr, -1))
        elif self.use_attn:
            input_video_o = video_o.view(bsize, numc * numf * numr, -1)
            input_mask = torch.ones(bsize, numc * numf * numr).cuda()
            video_o = self.attn(input_video_o, input_video_o, input_video_o, input_mask)[0]
        elif self.use_conv:
            video_o = self.conv(video_o.view(bsize, numc * numf * numr, -1))

        video_o = video_o.view(bsize, numc, numf, numr, -1)

        q_feat, w_feat = self.get_clip_txt_embedding(question_clip)

        video_f_norm = video_f 
        ...
        if answer is not None:
            if labels is not None and self.use_cacr:
                return fusion_proj, answer_proj, (intra_text_kl_div + intra_video_kl_div)/2.0
            else:
