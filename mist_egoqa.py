...
class ISTA(nn.Module):
    def __init__(self, feature_dim, word_dim, Q, N, d_model, dropout, d_ff, h, topk=6, topj=12, num_frames_in_feature_file=32):
...
class MIST_VideoQA(nn.Module):
    def __init__(
        ...
        self.ISTA = [ISTA(feature_dim=feature_dim, word_dim=word_dim, Q=Q, N=N,
                          d_model=d_model, dropout=dropout, d_ff=d_ff, h=h, topk=self.topk, topj=self.topj, num_frames_in_feature_file=self.num_frames_in_feature_file)]
        ...
        self.video_projection = nn.Linear(1536, feature_dim)
