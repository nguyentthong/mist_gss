...
def train(model, train_loader, a2v, optimizer, criterion, scheduler, epoch, args, tokenizer):
    ...
    for i, batch in enumerate(tqdm(train_loader)):
        ...
        macs, params = profile(model, inputs=(video, question, question_clip, None, answer.cuda(), seq_len, video_mask, question_mask,))
        print("Number of FLOPs: {}G".format(macs * 0.5 / 10**9))
        if not args.mc:
            model.module._compute_answer_embedding(a2v)
            predicts = model(
                video,
